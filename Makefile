PROJECT_NAME := F5 BigIP Package
include build/common.mk

PACK             := f5bigip
PACKDIR          := sdk
PROJECT          := github.com/pulumi/pulumi-f5bigip
NODE_MODULE_NAME := @pulumi/f5bigip
...
build::
	go install -ldflags "-X github.com/pulumi/pulumi-f5bigip/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${TFGEN}
	go install -ldflags "-X github.com/pulumi/pulumi-f5bigip/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${PROVIDER}
...
install::
	GOBIN=$(PULUMI_BIN) go install -ldflags "-X github.com/pulumi/pulumi-f5bigip/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${PROVIDER}
